 ...
 */
export const generateCryptoPlaceholder = (symbol: string, size = 100): string => {
  try {
    ...
    const hue = Array.from(cleanSymbol).reduce((acc, char) => acc + char.charCodeAt(0), 0) % 360
...
export const generateWalletPlaceholder = (address: string, size = 64): string => {
  try {
    if (!address) return ""
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}">
        ...
        <circle cx="${size / 2}" cy="${size / 2}" r="${size / 3}" fill="${darkColor}" />
        <text x="${size / 2}" y="${size / 2}" font-family="monospace" font-size="${
          size / 4
        }" fill="white" text-anchor="middle" dominant-baseline="central">${address.slice(0, 2)}</text>
      </svg>
    `
 ...
 * @returns Un URL de date pentru SVG
 */
export const generatePlaceholder = (text: string, size = 32, type: "crypto" | "wallet" = "crypto"): string => {
  return type === "crypto" ? generateCryptoPlaceholder(text, size) : generateWalletPlaceholder(text, size)
}
