...
export const fetchAllCryptosData = async (): Promise<Cryptocurrency[]> => {
  ...
  try {
    ...
    throw new Error("Failed to load cryptocurrency data. Please try again later.")
  }
}
...
export const fetchWhaleTransactions = async (page = 1, threshold = 100): Promise<WhaleTransactionsResponse> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/whale-transactions?filterValue=300`, {
