...
import type { FearGreedIndexData } from "../../types" // Dacă ai un tip definit pentru aceasta
export const fetchSentimentData = async (
  ...
  try {
    ...
    const sentimentTimestamps = data.map((item) => format(new Date(item.timestamp * 1000), "MMM dd, yyyy"))
