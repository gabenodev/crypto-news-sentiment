...
export const movingAverage = (data: ChartDataPoint[], period: number, key: string): ChartDataPoint[] => {
  ...
  return data.map((item, index) => {
    ...
    const sum = data.slice(index - period + 1, index + 1).reduce((acc, val) => acc + val.price, 0)
