...
import { movingAverage, calculateMinPrice, calculateMaxPrice, calculateAveragePrice } from "../Indicators/MAindicators"
...
function PriceChart({ coinId }: PriceChartProps): JSX.Element {
  ...
  if (loading) {
    return (
      <div
        className={`w-full ${
          fullscreen ? "fixed inset-0 z-50 bg-white dark:bg-gray-900 p-6" : "max-w-7xl mx-auto p-4"
        }`}
      >
        <div className="flex flex-col items-center justify-center h-80 bg-white dark:bg-gray-800 rounded-xl shadow-md p-8">
          ...
          <p className="text-gray-600 dark:text-gray-300 text-lg font-medium">Loading chart data...</p>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">Fetching the latest price information</p>
        </div>
      </div>
    )
  }
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center h-80 bg-white dark:bg-gray-800 rounded-xl shadow-md p-8">
          ...
          <h3 className="text-xl font-semibold text-gray-800 dark:text-white mb-2">Error Loading Chart</h3>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
          ...
          {payload.map((entry, index) => (
            <div key={index} className="flex items-center justify-between mb-1">
              ...
              <span className="text-sm font-bold ml-4" style={{ color: entry.color }}>
                {entry.name === "Price" ? "$" : ""}
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-dark-secondary">
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-dark-text-primary flex items-center">
              ...
              <span className="ml-2 text-sm font-medium bg-teal-100 dark:bg-teal-900/30 text-teal-800 dark:text-teal-300 px-2 py-0.5 rounded-full">
                {timeframeOptions.find((option) => option.value === days)?.label || "24h"}
              </span>
            </h2>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <select
                value={days.toString()}
                onChange={(e) => setDays(Number(e.target.value) || e.target.value)}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center">
              ...
              <div className="flex space-x-4">
                <div>
                  <div className="text-xs font-medium text-gray-500 dark:text-gray-400">24h Low</div>
                  <div className="text-sm font-semibold text-red-500">${formatPrice(minPrice)}</div>
                </div>
                <div>
                  <div className="text-xs font-medium text-gray-500 dark:text-gray-400">24h High</div>
                  <div className="text-sm font-semibold text-green-500">${formatPrice(maxPrice)}</div>
                </div>
              </div>
            </div>
              ...
              {Object.entries(movingAverages)
                ...
                .map(([key]) => (
                  <div
                    ...
                    style={{
                      backgroundColor: `${maColors[key as keyof typeof maColors]}20`,
              ...
              <div className="p-6 pt-4">
                <div className="h-[400px] md:h-[500px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={dataWithMAs6} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}>
                      <defs>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="#eee" strokeOpacity={0.3} vertical={false} />
                      <XAxis
                        dataKey="time"
              ...
              <div className="p-6 pt-4">
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    Current RSI:{" "}
                    <span className="font-medium">{Math.round(rsiData[rsiData.length - 1]?.rsi || 0)}</span>
                  </p>
                </div>

                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={rsiData} margin={{ top: 10, right: 30, left: 20, bottom: 10 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#eee" strokeOpacity={0.3} vertical={false} />
                      <XAxis
                        dataKey="time"
                        tick={{ fill: "#6b7280", fontSize: 11 }}
                      ...
                      />
                      <Tooltip
                        formatter={(value) => [`${Math.round(value as number)}`, "RSI"]}
                      ...
                      />
                      <ReferenceLine y={50} stroke="#9ca3af" strokeDasharray="3 3" strokeOpacity={0.4} />
                      <ReferenceLine
                        y={70}
                        stroke="#ef4444"
                        strokeDasharray="3 3"
                ...
                <div className="mt-6 bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg">
                  <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Understanding RSI</h4>
                  <p className="text-xs text-gray-600 dark:text-gray-400">
                    The Relative Strength Index (RSI) is a momentum oscillator that measures the speed and change of
                    price movements. RSI oscillates between 0 and 100. Traditionally, RSI is considered overbought when
                    above 70 and oversold when below 30. Signals can also be generated by looking for divergences,
              ...
              <div className="p-6 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white dark:bg-gray-700/30 p-5 rounded-xl border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      {Object.entries(movingAverages).map(([key, value]) => (
                        <button
                          key={key}
                          onClick={() => toggleMovingAverage(key as keyof typeof movingAverages)}
                          ...
                          style={
                            value
                              ? {
                                  backgroundColor: maColors[key as keyof typeof maColors],
                                }
                              : {}
                          }
                        >
                    ...
                    <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">
                      <p>
                        Moving averages help identify trends by smoothing out price fluctuations over specific time
                        periods.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-700/30 p-5 rounded-xl border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">
                      <p>
                        Reference lines help visualize important price levels like minimum, maximum, and average values.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-700/30 p-5 rounded-xl border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded-lg">
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Minimum</div>
                      ...
                      <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Maximum</div>
                    ...
                    <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">
                      <p>Summary of key price points during the selected time period.</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-gray-700/30 p-5 rounded-xl border border-gray-200 dark:border-gray-600">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{priceData.length}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-gray-600 dark:text-gray-400">First Data Point:</span>
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">
                          {priceData[0]?.time ? new Date(priceData[0].time).toLocaleString() : "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-sm text-gray-600 dark:text-gray-400">Last Data Point:</span>
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">
                          {priceData[priceData.length - 1]?.time
                            ? new Date(priceData[priceData.length - 1].time).toLocaleString()
                            : "N/A"}
                        </span>
                      </div>
                    </div>
