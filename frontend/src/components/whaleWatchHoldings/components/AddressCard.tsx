...
const AddressCard: React.FC<AddressCardProps> = ({ address, selectedChain, getExplorerUrl, className = "" }) => {
  ...
  return (
    <div
      className={`bg-gray-50 dark:bg-dark-tertiary rounded-lg p-3 mb-3 border border-gray-200 dark:border-gray-700 w-full ${className}`}
    >
      <div className="flex justify-between items-center mb-1">
        <span className="text-sm text-gray-500 dark:text-dark-text-secondary">Address</span>
      ...
      <div className="bg-white dark:bg-dark-secondary rounded-lg px-2 py-1.5 font-mono text-xs text-gray-700 dark:text-dark-text-primary break-all">
        {address}
        {copySuccess && <span className="text-xs text-teal-500 dark:text-teal-400 ml-2">Copied!</span>}
      </div>
    </div>
  )
}
