...
// Import types
import type { WalletHoldingsProps, TokenData } from "./types";
  ...
  useEffect(() => {
    ...
    try {
      ...
      processedTokens = processedTokens
        .map((token) => normalizeTokenValue(token, totalValue))
        .map((token) => ({
          ...token,
          formattedBalance: token.formattedBalance || 0, // Ensure formattedBalance is always a number
          value: token.value || 0, // Ensure value is always a number
        }));
