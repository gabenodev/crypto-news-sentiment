...
export default function WhaleTransactions(): JSX.Element {
  ...
  const { transactions, totalPages, loading } = useWhaleTransactions(page, itemsPerPage)
  ...
  useEffect(() => {
    const fetchCryptoData = async () => {
      try {
        ...
        const cryptoMap = data.reduce((acc: Record<string, CryptoData>, crypto: CryptoData) => {
