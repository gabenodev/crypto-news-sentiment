    ...
    <div className="flex flex-col items-center">
      <div className="w-48 h-48 relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-sm font-medium mt-1 text-gray-500 dark:text-gray-400">{getSentimentLabel(value)}</span>
        </div>
      </div>
      <div className="mt-4 flex justify-between w-full max-w-xs">
        {[0, 25, 50, 75, 100].map((mark) => (
          <span key={mark} className="text-xs font-medium text-gray-500 dark:text-gray-400">
            {mark}
          </span>
        ))}
      </div>
